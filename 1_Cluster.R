#DGP: Standard DGP with No Hidden Confounding and Clustering
...
setwd("C:\\Users\\eminu\\Desktop\\Bachelor Thesis Finish\\Cluster")
...
gen_data.panelclust = function(t, t.test, d, noise, entity, rho){
  ...
  for(j in 1:entity){
    ...
    #so one can cluster for time
    time = seq(1:t)
    
    #Make a matrix consisting of Y, W, X, tau, entity.fe and time
  ...
  #Return panel data
  data.panelclust = list(X = res[,Xnam],
                         X.test = res[,Xtestnam],
                         W = res[,"W"],
                         Y = res[,"Y"],
                         tau = res[,"tau"],
...
#SIMULATION WITHOUT HIDDEN CONFOUNDING AND CLUSTERING
simulation.panelclust = function(t, t.test, d, noise, entity, clusters, rho){ 
