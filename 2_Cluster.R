...
run.cluster = function(t, t.test, d, noise, entity, rho, n.sim, seed) {
  ...
  #Combine output
  output = rbind.data.frame(output, colMeans(results, na.rm = T))
...
for (j in n){
  for(i in d){
    ...
    #Run
    result = run.cluster(t = j, t.test = j, d = i, noise = 1, entity = 10, rho = 0.2, n.sim = 100, seed = 10)
    save.image(paste(".\\Results\\Cluster",j,i,".RData", sep = ""))
  }
}